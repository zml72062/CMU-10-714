...
def test_softmax_regression_epoch():
    ...
    dTheta = -nd.Gradient(lambda Th: softmax_loss(X @
                          Th.reshape(5, 3), y))(Theta)
    softmax_regression_epoch(X, y, Theta, lr=1.0, batch=50)
    np.testing.assert_allclose(dTheta.reshape(
        5, 3), Theta, rtol=1e-4, atol=1e-4)
...
def test_nn_epoch():
    ...
    np.testing.assert_allclose(dW1.reshape(
        5, 10), W1_0-W1, rtol=1e-4, atol=1e-4)
    np.testing.assert_allclose(dW2.reshape(
        10, 3), W2_0-W2, rtol=1e-4, atol=1e-4)
