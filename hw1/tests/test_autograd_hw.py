...
def test_negate_forward():
    np.testing.assert_allclose(ndl.negate(ndl.Tensor(
        [[1.45, 0.55]])).numpy(), np.array([[-1.45, -0.55]]))
...
def submit_forward():
    ...
    mugrade.submit(ndl.broadcast_to(ndl.Tensor(
...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    if not backward:
        out = f(*args, **kwargs)
        computed_grads = [x.numpy() for x in out.op.gradient_as_tuple(
...
def test_divide_scalar_backward():
    gradient_check(ndl.divide_scalar, ndl.Tensor(
        np.random.randn(5, 4)), scalar=np.random.randn(1))
...
def test_matmul_batched_backward():
    gradient_check(ndl.matmul, ndl.Tensor(np.random.randn(
        6, 6, 5, 4)), ndl.Tensor(np.random.randn(6, 6, 4, 3)))
    gradient_check(ndl.matmul, ndl.Tensor(np.random.randn(
        6, 6, 5, 4)), ndl.Tensor(np.random.randn(4, 3)))
    gradient_check(ndl.matmul, ndl.Tensor(np.random.randn(5, 4)),
...
def test_broadcast_to_backward():
    ...
    gradient_check(ndl.broadcast_to, ndl.Tensor(
        np.random.randn(1,)), shape=(3, 3, 3))
    gradient_check(ndl.broadcast_to, ndl.Tensor(
        np.random.randn()), shape=(3, 3, 3))
    gradient_check(ndl.broadcast_to, ndl.Tensor(
        np.random.randn(5, 4, 1)), shape=(5, 4, 3))
...
def submit_backward():
    np.random.seed(0)
    out = gradient_check(ndl.divide, ndl.Tensor(
        np.random.randn(3, 5)), ndl.Tensor(6 + np.random.randn(3, 5)))
    print(out)
    mugrade.submit(out)
    mugrade.submit(gradient_check(ndl.divide_scalar, ndl.Tensor(
        np.random.randn(3, 5)), scalar=np.random.randn(1)))
    mugrade.submit(gradient_check(ndl.matmul, ndl.Tensor(
        np.random.randn(1, 5)), ndl.Tensor(np.random.randn(5, 1))))
    mugrade.submit(gradient_check(ndl.matmul, ndl.Tensor(
        np.random.randn(2, 4)), ndl.Tensor(np.random.randn(4, 2))))
    mugrade.submit(gradient_check(ndl.matmul, ndl.Tensor(
        np.random.randn(2, 4)), ndl.Tensor(np.random.randn(7, 4, 2))))
    mugrade.submit(gradient_check(ndl.matmul, ndl.Tensor(
        np.random.randn(3, 2, 1)), ndl.Tensor(np.random.randn(3, 3, 1, 2))))
    mugrade.submit(gradient_check(ndl.matmul, ndl.Tensor(
        np.random.randn(2, 4)), ndl.Tensor(np.random.randn(2, 4, 4, 2))))
    mugrade.submit(gradient_check(ndl.reshape, ndl.Tensor(
    ...
    mugrade.submit(gradient_check(ndl.transpose, ndl.Tensor(
        np.random.randn(3, 2, 4)), axes=(0, 2)))
    mugrade.submit(gradient_check(ndl.broadcast_to,
                   ndl.Tensor(np.random.randn(7, 1)), shape=(7, 7)))
    mugrade.submit(gradient_check(ndl.broadcast_to,
                   ndl.Tensor(np.random.randn(1, 5)), shape=(5, 5)))
    mugrade.submit(gradient_check(ndl.broadcast_to, ndl.Tensor(
        np.random.randn(1,)), shape=(4, 4, 4)))
    mugrade.submit(gradient_check(ndl.broadcast_to,
                   ndl.Tensor(np.random.randn()), shape=(1, 3, 6)))
    mugrade.submit(gradient_check(ndl.broadcast_to, ndl.Tensor(
        np.random.randn(4, 4, 1)), shape=(4, 4, 6)))
    mugrade.submit(gradient_check(
        ndl.summation, ndl.Tensor(np.random.randn(3, 2, 1))))
    mugrade.submit(gradient_check(
        ndl.summation, ndl.Tensor(np.random.randn(3, 6)), axes=(1,)))
    mugrade.submit(gradient_check(
        ndl.summation, ndl.Tensor(np.random.randn(7,)), axes=(0,)))
    mugrade.submit(gradient_check(ndl.summation, ndl.Tensor(
        np.random.randn(7, 8)), axes=(0, 1)))
    mugrade.submit(gradient_check(ndl.summation, ndl.Tensor(
        np.random.randn(5, 4, 5)), axes=(0, 1, 2)))
...
def test_topo_sort():
    ...
    soln = np.array([np.array([[1.4335016, 0.30559972], [0.08130171, -1.15072371]]),
                     np.array([[1.34571691, -0.95584433],
                              [-0.99428573, -0.04017499]]),
                     np.array([[1.6252339, -1.38248184],
                              [1.25355725, -0.03148146]]),
                     np.array([[2.97095081, -2.33832617],
                              [0.25927152, -0.07165645]]),
                     np.array([[-1.4335016, -0.30559972],
                              [-0.08130171, 1.15072371]]),
                     np.array([[1.53744921, -2.64392589],
                              [0.17796981, 1.07906726]]),
...
def submit_compute_gradient():
    ...
    a = ndl.Tensor(np.array([[0.4736625, 0.06895066, 1.36455087, -0.31170743, 0.1370395],
                             [0.2283258, 0.72298311, -1.20394586, -
                                 1.95844434, -0.69535299],
                             [0.18016408, 0.0266557, 0.80940201, -
                                 0.45913678, -0.05886218],
...
def test_softmax_loss_ndl():
    ...
    np.testing.assert_allclose(softmax_loss(
        Z, y).numpy(), 2.3025850, rtol=1e-6, atol=1e-6)
    Z = ndl.Tensor(np.random.randn(y.shape[0], 10).astype(np.float32))
    np.testing.assert_allclose(softmax_loss(
        Z, y).numpy(), 2.7291998, rtol=1e-6, atol=1e-6)
...
def submit_softmax_loss_ndl():
    ...
    y_one_hot[np.arange(y.size), y] = 1
    y = ndl.Tensor(y_one_hot)
    mugrade.submit(softmax_loss(ndl.Tensor(
        np.zeros((y.shape[0], 10)).astype(np.float32)), y).numpy())
    np.random.seed(0)
    mugrade.submit(softmax_loss(ndl.Tensor(np.random.randn(
        y.shape[0], 10).astype(np.float32)), y).numpy())
...
def test_nn_epoch_ndl():
    ...
    np.testing.assert_allclose(dW1.reshape(
        5, 10), W1_0-W1.numpy(), rtol=1e-4, atol=1e-4)
    np.testing.assert_allclose(dW2.reshape(
        10, 3), W2_0-W2.numpy(), rtol=1e-4, atol=1e-4)
